...
cube = drawRect(pMatrix, screen, Vertex(0, 0, -10), 5, 5, 5, red, blue, green, yellow, orange, purple)

def show_thing(root, screen, pM):
    root.destroy()
    running = True
    while running:
        ...
        screen.fill(black)
        cube.draw(screen, pM)
        cube.rotate("x", Vertex(0, 0, -10), 2)
        cube.rotate("y", Vertex(0, 0, -10), 4)
        sleep(0.01)
        py.display.flip()